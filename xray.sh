...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        echo ""
        echo " Xray install script，make sure："
        colorEcho ${YELLOW} "  1. one domain"
        colorEcho ${YELLOW} "  2. domain DNS parse to current ip（${IP}）"
        colorEcho ${BLUE} "  3. if   xray.pem 和 xray.key in `/root`，ignore 2"
        echo " "
        read -p " sure enter y，else enter other key：" answer
        if [[ "${answer,,}" != "y" ]]; then
            exit 0
        fi
        ...
        if [[ -f ~/xray.pem && -f ~/xray.key ]]; then
            ...
            if [[ -z "${domain_ip}" ]]; then
                colorEcho ${BLUE}  "${DOMAIN} 解析结果：${domain_ip}"
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        echo "   1) static web(location /usr/share/nginx/html)"
        #echo "   2) 小说站(随机选择)"
        #echo "   3) 美女站(https://imeizi.me)"
        #echo "   4) VPS优惠博客(https://www.lowendtalk.com)"
        echo "   5) 自定义反代站点(需以http或者https开头)"
        read -p "  请选择伪装网站类型[默认:static web]" answer
        if [[ -z "$answer" ]]; then
            PROXY_URL=""
        else
            case $answer in
        ...
        echo ""
        colorEcho $BLUE "  allow robot? 是否允许搜索引擎爬取网站？[默认：不允许]"
        echo "    y) allow 允许，会有更多ip请求网站，但会消耗一些流量，vps流量充足情况下推荐使用"
        echo "    n) forbit 不允许，爬虫不会访问网站，访问ip比较单一，但能节省vps流量"
    ...
    echo ""
    read -p " 是否install BBR(默认install )?[y/n]:" NEED_BBR
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        # 先要注册ZeroSSl
        ~/.acme.sh/acme.sh  --register-account -m xray@163.com --server zerossl
        if [[ "$BT" = "false" ]]; then
	    systemctl stop nginx
            ~/.acme.sh/acme.sh   --issue --insecure -d $DOMAIN -k ec-256 --force  --reloadcmd "systemctl restart nginx"  --standalone
        else
	    nginx -s stop
            ~/.acme.sh/acme.sh   --issue --insecure -d $DOMAIN -k ec-256 --force  --reloadcmd "nginx -c /www/server/nginx/conf/nginx.conf || { echo -n ''; }"  --standalone
        fi
        ...
        CERT_FILE="/usr/local/etc/xray/${DOMAIN}.pem"
        KEY_FILE="/usr/local/etc/xray/${DOMAIN}.key"
        ~/.acme.sh/acme.sh  --installcert -d $DOMAIN --ecc --force \
            --keypath       $KEY_FILE  \
            --fullchainpath $CERT_FILE \
...
install() {
    ...
    if [[ $? -ne 0 ]]; then
        colorEcho $RED " unzip install failed，please check net is working"
        exit 1
    fi
    ...
    RETVAL="$?"
    if [[ $RETVAL == 0 ]]; then
        colorEcho $BLUE " Xray lastest ${CUR_VER} is alread installed "
    elif [[ $RETVAL == 3 ]]; then
        exit 1
    else
        colorEcho $BLUE " install Xray ${NEW_VER} ，arch: $(archAffix)"
        installXray
    fi
...
update() {
    res=`status`
    if [[ $res -lt 2 ]]; then
        colorEcho $RED " Xray未install ，请先install ！"
        return
    fi
    ...
    RETVAL="$?"
    if [[ $RETVAL == 0 ]]; then
        colorEcho $BLUE " Xray最新版 ${CUR_VER} 已经install "
    elif [[ $RETVAL == 3 ]]; then
        exit 1
    else
        colorEcho $BLUE " install Xray ${NEW_VER} ，架构$(archAffix)"
        installXray
        stop
        start

        colorEcho $GREEN " 最新版Xray install 成功！"
    fi
}
...
start() {
    ...
    res=`ss -nutlp| grep ${port} | grep -i xray`
    if [[ "$res" = "" ]]; then
        colorEcho $RED " Xray start failed，please check logs or check port used ！"
    else
        colorEcho $BLUE " Xray started"
    fi
}
...
restart() {
    res=`status`
    if [[ $res -lt 2 ]]; then
        colorEcho $RED " Xray not install ，please install first！"
        return
    fi
...
outputVmess() {
  ...
  \"tls\":\"\"
    ...
    echo -e "   ${BLUE}订阅内容:${PLAIN} $RED$subs$PLAIN"

    uuid=`cat /proc/sys/kernel/random/uuid`
    filename = ${uuid: 0:8}.txt
    file=/usr/share/nginx/html/${uuid: 0:8}.txt
    suburl=ssr.${domain}/${filename}
    echo $subs > ${file}
    echo -e "   ${BLUE}订阅文件:${PLAIN} $RED${file}$PLAIN"
    echo -e "   ${BLUE}订阅连接:${PLAIN} $RED${suburl}$PLAIN"
}
...
showInfo() {
    ...
    echo ""
    echo -n -e " ${BLUE}Xray status：${PLAIN}"
    statusText
    echo -e " ${BLUE}Xray configuration file: ${PLAIN} ${RED}${CONFIG_FILE}${PLAIN}"
    colorEcho $BLUE " Xray configuration："
...
showLog() {
    res=`status`
    if [[ $res -lt 2 ]]; then
        colorEcho $RED " Xray uninstall ，please install ！"
        return
    fi
...
menu() {
    clear
    echo "#############################################################"
    echo -e "#                     ${RED}Xray install script${PLAIN}                      #"
    ...
    echo -e "  ${GREEN}7.${PLAIN}   install Xray-${BLUE}VLESS+WS+TLS${PLAIN}${RED}(可过cdn)${PLAIN}"
    echo -e "  ${GREEN}8.${PLAIN}   install Xray-${BLUE}VLESS+TCP+XTLS${PLAIN}${RED}(recommend)${PLAIN}"
    echo -e "  ${GREEN}9.${PLAIN}   install ${BLUE}trojan${PLAIN}${RED}(recommend)${PLAIN}"
    ...
    echo " -------------"
    echo -e "  ${GREEN}16.${PLAIN}  show Xray configuration"
    echo -e "  ${GREEN}17.${PLAIN}  show xray log"
    echo " -------------"
    ...
    case $answer in
        ...
        17)
            ...
            ;;
        *)
            colorEcho $RED " please enter right number！"
            exit 1
            ;;
    esac
}
...
case "$action" in
    ...
    *)
        echo " args error:"
        echo " use: `basename $0` [menu|update|uninstall|start|restart|stop|showInfo|showLog]"
        ;;
esac